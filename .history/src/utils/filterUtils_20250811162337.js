...
export const buildMovieDiscoverURL = (filters, page = 1, apiKey) => {
  ...
  if (filters.yearRange) {
    if (filters.yearRange.min) {
      params.append("primary_release_date.gte", `${filters.yearRange.min}-01-01`);
    }
    if (filters.yearRange.max) {
      params.append("primary_release_date.lte", `${filters.yearRange.max}-12-31`);
    }
  }
...
export const buildTVDiscoverURL = (filters, page = 1, apiKey) => {
  ...
  if (filters.yearRange) {
    if (filters.yearRange.min) {
      params.append("first_air_date.gte", `${filters.yearRange.min}-01-01`);
    }
    if (filters.yearRange.max) {
      params.append("first_air_date.lte", `${filters.yearRange.max}-12-31`);
    }
  }
...
export const getActiveFilterCount = (filters) => {
  ...
  if (filters.yearRange && (filters.yearRange.min || filters.yearRange.max)) count++;

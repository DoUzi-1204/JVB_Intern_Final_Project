...
// Hàm tạo URL với query parameters cho trang filter
export const createFilterURL = (filters, page = 1) => {
  const searchParams = new URLSearchParams();
  
  if (filters.contentType) {
    searchParams.set('type', filters.contentType);
  }
  if (filters.genres && filters.genres.length > 0) {
    searchParams.set('genres', filters.genres.join(','));
  }
  if (filters.countries && filters.countries.length > 0) {
    searchParams.set('countries', filters.countries.join(','));
  }
  if (filters.selectedYear) {
    searchParams.set('year', filters.selectedYear.toString());
  }
  if (filters.ageRatings && filters.ageRatings.length > 0) {
    searchParams.set('age', filters.ageRatings.join(','));
  }
  ...
  return `/filter${searchParams.toString() ? `?${searchParams.toString()}` : ''}`;
};

// Hàm parse URL parameters thành filters object
export const parseFiltersFromURL = (searchString) => {

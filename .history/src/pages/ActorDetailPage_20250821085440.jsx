...
export default function ActorDetailPage() {
	const { id } = useParams();
	const { actor, credits, loading, error } = useActorDetail(id);

	if (loading) return <div className="text-center text-white">Đang tải...</div>;
	if (error) return <div className="text-center text-red-500">Lỗi tải dữ liệu.</div>;
	if (!actor) return <div className="text-center text-gray-400">Không tìm thấy diễn viên.</div>;

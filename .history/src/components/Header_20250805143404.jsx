      ...
      <div className="w-full px-4 flex items-center justify-between gap-4">
        ...
        <nav className="flex items-center gap-6 flex-shrink-0">
          ...
          <div className="relative" ref={genreDropdownRef}>
            ...
            {/* Genre Dropdown Menu */}
            {showGenreDropdown && (
              <div className="absolute top-full left-0 mt-2 w-[600px] bg-zinc-900 border border-gray-700 rounded-lg shadow-xl z-50 overflow-hidden">
                {loading ? (
                    ...
                    <div className="mb-6">
                      <div className="grid grid-cols-4 gap-2">
                        {genres.movies.map((genre) => (
                          <button
                            key={`movie-${genre.id}`}
                            className="text-left text-white text-sm py-2 px-3 rounded hover:bg-gray-700 hover:text-yellow-400 transition-colors duration-200"
                            onClick={() => {
                              // Handle movie genre click
                              console.log('Movie genre clicked:', genre);
                              setShowGenreDropdown(false);
                            }}
                          >
                            {genre.name}
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        {genres.tv.map((genre) => (
                          <button
                            ...
                            className="text-left text-white text-sm py-2 px-3 rounded hover:bg-gray-700 hover:text-yellow-400 transition-colors duration-200"
                            onClick={() => {
                              // Handle TV genre click
                              console.log('TV genre clicked:', genre);
                              setShowGenreDropdown(false);
                            }}
                          >
                            {genre.name}

...
const Login = ({ logoSrc = "/logo.png", bgSrc = "/login-left.jpg", onClose }) => {
			...
			<div className="w-[90%] max-w-4xl bg-transparent rounded-lg overflow-hidden shadow-2xl grid grid-cols-12">
				...
				>
					...
					<div className="absolute left-6 bottom-6 flex items-center gap-3 text-white">
						...
						<div>
							<div className="text-2xl font-bold">RoPhim</div>
							<div className="text-sm opacity-80">Phim hay cả rổ</div>
						</div>
					</div>
				</div>
				...
				<div className="col-span-12 sm:col-span-5 bg-[#131428] p-8 relative">
					...
					<p className="text-sm text-gray-300 mb-6">
						Nếu bạn chưa có tài khoản, <span className="text-yellow-300">đăng ký ngay</span>
					</p>
					...
					<form onSubmit={handleSubmit} className="space-y-4">
						...
						{/* status box (example: 'Thành công!') */}
						{status === "success" && (
							<div className="flex items-center gap-3 bg-black border border-gray-700 p-3 rounded">
								<div className="w-8 h-8 bg-green-600 rounded-full flex items-center justify-center text-white">✓</div>
								<div className="text-sm text-white">Thành công!</div>
								<div className="ml-auto text-xs text-gray-300">Cloudflare</div>
							</div>
						)}
					...
					<div className="mt-4 text-center text-sm text-gray-400">Quên mật khẩu?</div>

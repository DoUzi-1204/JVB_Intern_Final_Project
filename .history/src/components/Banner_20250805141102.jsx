...
const Banner = () => {
  ...
  // Wrap fetch functions với useCallback
  ...
  const fetchMovieDetailsEn = useCallback(
    async (movieId) => {
      try {
        ...
        console.error("Error fetching English movie details:", error);
      }
    },
    [API_KEY, BASE_URL]
  );
  ...
  // useEffect với đầy đủ dependencies
  useEffect(() => {
    if (currentMovie) {
  ...
  const handleWatchMovie = () => {
    if (currentMovie) {
      // Temporarily navigate to movie detail page
      // In a real app, this would navigate to a watch page
        ...
        <div className="container mx-auto px-6 flex items-center justify-between h-full">
          ...
          <div className="flex-1 max-w-2xl space-y-3 text-left">
            ...
            <div className="flex items-center space-x-4 ">
              <button className="bg-gradient-to-r from-yellow-100 to-yellow-400 backdrop-blur-sm text-black px-6 py-3 rounded-full font-semibold hover:from-yellow-400 hover:to-yellow-500 transition-colors flex items-center space-x-2">
                <TiMediaPlay className="w-6 h-6" />
                <span>Xem phim</span>
              </button>

              <button className="border border-white/50 bg-gray-600/50 backdrop-blur-sm text-white px-6 py-3 rounded-full font-semibold transition-colors flex items-center space-x-2 hover:text-yellow-300">
                <FaCircleInfo className="w-6 h-6 transition-colors" />
                <span>Chi tiết</span>
              </button>

              {/* Nút yêu thích có tooltip phía dưới */}
              ...
              {/* Nút thêm vào danh sách có tooltip phía dưới */}
              <div className="relative group">
                <button className="border border-white/50 w-12 h-12 bg-gray-600/50 backdrop-blur-sm text-white rounded-full transition-colors flex items-center justify-center group-hover:text-yellow-300">
                  <HiMiniListBullet className="text-2xl transition-colors" />
                </button>
                <span className="absolute top-full mt-1 left-1/2 -translate-x-1/2 px-2 py-1 text-xs rounded bg-black text-white opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                  Thêm vào danh sách
                </span>

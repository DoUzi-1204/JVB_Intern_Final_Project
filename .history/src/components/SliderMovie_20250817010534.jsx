...
const SliderMovie = ({
  ...
  return (
    <div className="mb-8">
      ...
      {/* Slider Container */}
      <div className="relative group overflow-hidden"> {/* overflow-hidden để "cửa sổ" */}
        {loading ? (
          ...
          <>
            ...
            <Swiper
              ...
              watchSlidesProgress // để biết slide nào visible
              slideVisibleClass="slide-visible" // gán class cho slide visible
            >
              {data.map((item, index) => (
                <SwiperSlide
                  key={`${item.id}-${index}`}
                  virtualIndex={index}
                  className="h-auto transition-opacity duration-300 transform scale-95 opacity-0"
                >
                  <ItemCard
                    item={item}
                    mediaType={mediaType}

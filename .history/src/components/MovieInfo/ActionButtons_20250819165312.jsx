...
const ActionButtons = ({ isMovie, movieId, onRatingClick }) => {
  ...
  const handleComment = () => console.log("Mở bình luận");
    const handleRating = () => {
    if (typeof onRatingClick === 'function') onRatingClick();
    else console.log("Đánh giá phim");
    };
  ...
  return (
    <div className="flex flex-col md:flex-row items-center gap-2 xl:gap-4 w-full">
      ...
      >
        ...
        <span className="text-sm sm:text-lg">Xem ngay</span>
      </Link>
      ...
      <div className="flex flex-row flex-wrap md:flex-row items-center justify-center  gap-1 xl:gap-3 w-full md:w-auto">
        ...
        <button
          ...
          className="flex items-center gap-2 sm:gap-3 px-4 py-2 sm:px-5 sm:py-3 

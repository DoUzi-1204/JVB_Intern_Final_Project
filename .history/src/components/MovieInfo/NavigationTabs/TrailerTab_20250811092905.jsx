...
const TrailerTab = ({ videos }) => {
  ...
  // Debug: Log all videos to see what we have
  console.log("All videos from API:", videos.results);
  console.log("Trailer videos:", videos.results.filter(v => v.type === "Trailer"));

  // Filter videos: only get "Official Trailer" with Vietnamese and English
  const vietnameseVideos = videos.results.filter(
    (video) =>
      (video.iso_639_1 === "vi" || video.iso_639_1 === "vi_VN") &&
      video.type === "Trailer" &&
      video.name.toLowerCase().includes("official trailer")
  );

  const englishVideos = videos.results.filter(
    (video) =>
      (video.iso_639_1 === "en" || video.iso_639_1 === "en_US") &&
      video.type === "Trailer" &&
      video.name.toLowerCase().includes("official trailer")
  );

  const allVideos = [...vietnameseVideos, ...englishVideos];

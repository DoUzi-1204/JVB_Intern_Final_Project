...
const NavButtons = ({
  ...
  return (
    <div className="nav-tabs-container mt-5">
      {/* Navigation Buttons */}
      <div className="nav-tabs flex xs:justify-center md:justify-start border-b border-gray-700 gap-0 md:gap-2 mb-7 overflow-x-auto">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            className={`nav-tab flex items-center gap-2 xs:gap-3 px-2.5 xs:px-4 py-3 border-b-2 transition-all duration-300 whitespace-nowrap text-sm font-medium ${
              activeTab === tab.id

  ...
  if (!videos || !videos.results || videos.results.length === 0) {
    return (
      <div className="trailer-tab-container">
        <h3 className="text-xl font-bold text-yellow-400 mb-6">Trailer</h3>
        <p className="text-gray-400">Chưa có trailer nào.</p>
      </div>
    );
  }

  // Filter videos: prioritize Vietnamese, then English, then any
  const vietnameseVideos = videos.results.filter(video => 
    video.iso_639_1 === 'vi' && (video.type === 'Trailer' || video.type === 'Teaser')
  );
  
  const englishVideos = videos.results.filter(video => 
    video.iso_639_1 === 'en' && (video.type === 'Trailer' || video.type === 'Teaser')
  );
  
  const otherVideos = videos.results.filter(video => 
    video.iso_639_1 !== 'vi' && video.iso_639_1 !== 'en' && (video.type === 'Trailer' || video.type === 'Teaser')
  );

  const allVideos = [...vietnameseVideos, ...englishVideos, ...otherVideos];
  ...
  return (
    <div className="trailer-tab-container">
      ...
      {allVideos.length > 0 ? (
        ...
        <p className="text-gray-400">Không có trailer hoặc teaser nào.</p>
      )}
        ...
        >
          <div className="relative w-full max-w-4xl">
            ...
            <div className="mt-4 text-center">
              <h3 className="text-white text-xl font-semibold mb-2">{selectedVideo.name}</h3>
              <div className="flex justify-center gap-4 text-sm text-gray-300">
                <span>Loại: {selectedVideo.type}</span>
                <span>Ngôn ngữ: {selectedVideo.iso_639_1?.toUpperCase()}</span>
                {selectedVideo.size && <span>Chất lượng: {selectedVideo.size}p</span>}
              </div>
            </div>
          </div>
        </div>

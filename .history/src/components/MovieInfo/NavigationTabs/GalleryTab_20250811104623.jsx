...
const GalleryTab = ({ images }) => {
  ...
  const filteredBackdrops = images?.backdrops
    ?.filter(img => !img.iso_639_1 || ['vi', 'en'].includes(img.iso_639_1))
    ?.slice(0, 5) || [];
    
  const filteredPosters = images?.posters
    ?.filter(img => !img.iso_639_1 || ['vi', 'en'].includes(img.iso_639_1))
    ?.slice(0, 5) || [];

  const hasImages = filteredBackdrops.length > 0 || filteredPosters.length > 0;
  
  const openModal = (image) => setSelectedImage(image);
  const closeModal = () => setSelectedImage(null);

  const ImageCard = ({ image, type, index, className }) => (
    <div
      key={`${type}-${index}`}
    ...
    >
      <img
        src={`${IMAGE_BASE_URL}/${type === 'backdrop' ? 'w500' : 'w342'}${image.file_path}`}
        alt={`${type} ${index + 1}`}
  ...
  return (
    <div className="gallery-tab-container">
      <h3 className="text-lg font-medium text-white mb-6 text-left">Hình ảnh</h3>

...
const CastTab = ({ credits }) => {
  ...
  return (
    <div className="cast-tab-container">
      ...
      <div className="cast-grid grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-4 xl:grid-cols-5 gap-3">
        {credits.cast.map((actor, index) => (
          <button
            key={`${actor.id}-${index}`}
            type="button"
            onClick={() => navigate(`/actor/${actor.id}`)}
            className="cast-item text-left focus:outline-none"
          ...
          >
            ...
            <div className="actor-info text-center">
              ...
              {/* Character Name */}
              <p className="text-red-300 text-xs line-clamp-2 leading-relaxed">

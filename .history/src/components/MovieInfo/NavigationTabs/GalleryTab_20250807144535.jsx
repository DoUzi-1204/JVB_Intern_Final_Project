...
const GalleryTab = ({ images }) => {
  ...
  // Filter images to only include Vietnamese and English
  const filteredBackdrops = images.backdrops?.filter(img => 
    !img.iso_639_1 || img.iso_639_1 === 'vi' || img.iso_639_1 === 'en'
  ) || [];
  
  const filteredPosters = images.posters?.filter(img => 
    !img.iso_639_1 || img.iso_639_1 === 'vi' || img.iso_639_1 === 'en'
  ) || [];

  const allImages = [
    ...filteredBackdrops.map(img => ({ ...img, type: 'backdrop' })),
    ...filteredPosters.map(img => ({ ...img, type: 'poster' }))
  ];
  ...
  return (
    <div className="gallery-tab-container">
      ...
      {filteredBackdrops.length > 0 && (
        <div className="mb-8">
          <h4 className="text-lg font-semibold text-white mb-4">Hình nền ({filteredBackdrops.length})</h4>
      ...
      {filteredPosters.length > 0 && (
        <div className="mb-8">
          <h4 className="text-lg font-semibold text-white mb-4">Poster ({filteredPosters.length})</h4>
          ...
          <div className="relative max-w-4xl max-h-full">
            ...
            <div className="absolute bottom-4 left-4 bg-black bg-opacity-70 text-white px-3 py-2 rounded-lg">
              <span className="text-sm">
                ...
                {selectedImage.iso_639_1 && ` • ${selectedImage.iso_639_1.toUpperCase()}`}
              </span>
            </div>
          </div>
        </div>

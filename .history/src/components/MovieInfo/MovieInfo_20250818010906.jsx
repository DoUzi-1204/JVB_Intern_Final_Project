...
const MovieInfo = ({ movieData, credits, isMovie }) => {
  ...
  return (
    <div className="bg-gray-900/95 backdrop-blur-sm rounded-3xl py-9 px-3 xl:px-9 text-white shadow-2xl">
      {/* Poster */}
      <div className="mb-7">
        <div className=" w-[170px] lg:w-[200px] xl:w-[250px] mx-auto">
          <Image404
            src={posterUrl}
            alt={displayTitleVi}
      ...
      <div className="mb-1 text-left">
        ...
        <div className="block lg:hidden mb-3 text-center">
          <button
            className="bg-yellow-400 text-black font-semibold px-4 py-2 rounded-lg shadow hover:bg-yellow-300 transition inline-flex items-center gap-2"
            onClick={() => setShowMeta((v) => !v)}
          >
            {showMeta ? "Ẩn thông tin phim" : "Thông tin phim"}
            <FaCaretDown className={`transition-transform duration-300 ${showMeta ? "rotate-180" : "rotate-0"}`} />
          </button>
        </div>
        {/* Container meta info: certification đến director/creator */}
        <div className={`space-y-3 ${showMeta ? "block" : "hidden"} lg:block`}>

      ...
      <div
        className={
          layout === "horizontal"
            ...
            : "w-48 h-72 bg-gray-800 rounded-lg animate-pulse"
        }
      >
        <div
          className={
  ...
  return (
    ...
    >
      {layout === "horizontal" ? (
        ...
        <Link to={`/${mediaType}/${item.id}`}>
          <div className="flex space-x-3 bg-gray-800/50 rounded-lg p-3 cursor-pointer transform transition-all duration-200 hover:bg-gray-700/50 hover:scale-105">
            ...
            <div className="flex-1 min-w-0">
              {/* Vietnamese Title */}
              {titleVi && (
                <h4 className="text-white font-medium text-sm mb-1 line-clamp-2 leading-tight hover:text-yellow-400 transition-colors">
                  {titleVi}
                </h4>
              )}
        ...
        <div className="w-48 cursor-pointer transform transition-transform duration-200 hover:scale-105 group">
          {/* Poster Image - Bọc trong Link */}
          <Link to={`/${mediaType}/${item.id}`}>
            <div className="relative aspect-[2/3] bg-gray-900 rounded-lg overflow-hidden mb-2">
              src={posterUrl}
              alt={titleVi || titleEn}
              className="w-full h-full object-cover" type="poster"
              loading="lazy" />
            </div>
          </Link>

          {/* Titles Below Poster - Cũng có thể bọc trong Link */}

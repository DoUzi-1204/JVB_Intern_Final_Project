    ...
    <div className={`relative ${className}`}>
      ...
      {showDropdown &&
        (displayMovies.length > 0 ||
          ...
          <div
            ref={dropdownRef}
            className="absolute top-full left-0 right-0 mt-2 bg-gray-900/95 backdrop-blur-sm border border-gray-600 rounded-lg shadow-2xl z-50 max-h-96 overflow-y-auto"
          >
            {/* Movies & TV Series */}
                ...
                <div className="space-y-2">
                  {[...displayMovies, ...displayTv].map((item) => (
                    <div
                      ...
                      className="flex items-start gap-3 p-2 rounded-md hover:bg-gray-700/50 cursor-pointer transition-colors duration-200"
                    >
                      {/* Poster */}
                        ...
                        <div className="flex items-center gap-2 text-xs text-gray-400">
                          ...
                          {item.vote_average > 0 && (
                            <span className="text-yellow-400 font-medium">
                              T{item.vote_average.toFixed(1).replace('.', '')}
                            </span>
                          )}
                          ...
                          {item.media_type === "tv" && (
                            <>
                              ...
                              {item.number_of_episodes && (
                                <span>Tập {item.number_of_episodes}</span>
                              )}
                            </>
                          )}
                        </div>
      ...
      {showDropdown &&
        ...
        displayPeople.length === 0 && (
          <div
            ref={dropdownRef}
            className="absolute top-full left-0 right-0 mt-2 bg-gray-900/95 backdrop-blur-sm border border-gray-600 rounded-lg shadow-2xl z-50 p-4"
          >

...
export default function ActorFilm({ credits, loading }) {
  ...
  return (
    <div className="min-w-3/4 p-0 rounded-lg ml-1">
      <h3 className="text-2xl font-medium text-left text-white mb-4">
        Phim liên quan
      </h3>
      <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-y-7 gap-x-3">
        {filtered.map((item) => (
          <ItemCard
            key={item.id}
            item={item}

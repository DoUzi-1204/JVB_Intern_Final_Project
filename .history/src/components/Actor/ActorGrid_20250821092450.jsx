...
const ActorGrid = ({ actors }) => {
  ...
  return (
    <div className="actor-grid-container">
      <h2 className="text-2xl text-white text-left font-medium mb-7 flex items-center">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
        {actors.map((actor) => (
          ...
          >
            {/* Profile Image */}
            <div className="aspect-[3/4] bg-gray-700 rounded-sm overflow-hidden mx-auto group">
              {actor.profile_path ? (
                <img
                  src={`${API_CONFIG.IMAGE_BASE_URL}/w300${actor.profile_path}`}
                  alt={actor.name}
                  className="w-full h-full object-cover transition duration-200 group-hover:brightness-110"
                />
              ) : (
            ...
            {/* Info */}
            <div className="p-2">
              <h3 className="text-white font-normal text-base truncate mb-3">
                {actor.name}
              </h3>
            </div>
          </div>

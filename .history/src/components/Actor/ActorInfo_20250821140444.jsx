...
export default function ActorInfo({ actor, loading }) {
  ...
  return (
    <div className="w-full max-w-xs p-0 flex flex-col bg-gray-900 rounded-2xl shadow-lg">
      ...
      <div className="w-full flex flex-col gap-3 self-start">
        <div className="text-base text-gray-300 self-start">
          <span className="font-semibold w-28 inline-block">Giới thiệu:</span>
          <span>{actor.biography ? actor.biography : "Đang cập nhật"}</span>
        </div>
        <div className="text-base text-gray-300 self-start">
          <span className="font-semibold w-28 inline-block">Giới tính:</span>
          <span>{genderMap[actor.gender]}</span>
        </div>
        <div className="text-base text-gray-300 self-start">
          <span className="font-semibold w-28 inline-block">Ngày sinh:</span>

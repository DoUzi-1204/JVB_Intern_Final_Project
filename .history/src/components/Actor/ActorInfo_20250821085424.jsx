...
export default function ActorInfo({ actor }) {
	...
	return (
		<div className="w-1/4 p-6 flex flex-col items-center bg-gray-900 rounded-lg shadow-lg">
			...
			/>
			<h2 className="text-xl font-bold text-white mb-2 text-center">{actor.name}</h2>
			<p className="text-gray-300 mb-1">Giới tính: <span className="font-semibold">{genderMap[actor.gender]}</span></p>
			<p className="text-gray-300 mb-1">Ngày sinh: <span className="font-semibold">{actor.birthday || "Không rõ"}</span></p>

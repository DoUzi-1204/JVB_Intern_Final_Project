      ...
      {loading ? (
        ...
        <ul className="space-y-4">
          {items.map((item, idx) => (
            <li
              key={item.id}
              className="flex items-center bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 rounded-xl px-7 py-5 backdrop-blur-md shadow-lg"
            >
              {/* Poster phim */}
              <Link to={`/${item.media_type}/${item.id}`}>
                <img
                  ...
                  className="w-20 h-32 object-cover rounded-xl shadow-lg flex-shrink-0"
                />
              </Link>
              {/* Thông tin phim */}

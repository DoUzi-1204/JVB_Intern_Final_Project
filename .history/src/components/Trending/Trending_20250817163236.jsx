      ...
      {loading ? (
        ...
        <ul className="space-y-4">
          {items.map((item, idx) => (
            <li
              key={item.id}
              className="relative flex items-center rounded-xl px-7 py-5 overflow-hidden"
              ...
              <div className="relative flex items-center w-full z-20">
                ...
                <Link to={`/${item.media_type}/${item.id}`}>
                  <img
                    ...
                    className="w-32 h-32 object-cover rounded-xl shadow-lg flex-shrink-0 border-spacing-2 border-white"
                  />
                </Link>
                {/* Thông tin phim */}

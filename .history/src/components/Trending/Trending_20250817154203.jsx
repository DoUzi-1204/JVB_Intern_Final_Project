...
const Trending = () => {
  ...
  return (
    <div className="bg-gray-900 p-4 rounded-lg mt-6 w-full max-h-[600px] overflow-y-auto">
      ...
      {loading ? (
        ...
        <ul className="space-y-4">
          {items.map((item, idx) => (
            <li
              key={item.id}
              className="flex items-center bg-gray-800 rounded-lg p-3 relative"
            >
              <span className="text-3xl font-bold text-blue-400 mr-6 w-8 text-center">
                {idx + 1}
              </span>
              <img
                src={

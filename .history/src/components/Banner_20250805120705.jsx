...
const Banner = () => {
  ...
  useEffect(() => {
    const fetchPopularMovies = async () => {
      console.log("Banner: Starting fetch with API_KEY:", !!API_KEY);
      // Check if API key is available
      if (!API_KEY) {
        console.log("Banner: No API key found");
        setError(
      ...
      try {
        ...
        console.log("Banner: Fetched movies:", popularMovies.length);
        setMovies(popularMovies);
        if (popularMovies.length > 0) {
          setCurrentMovie(popularMovies[0]);
          console.log("Banner: Set current movie:", popularMovies[0].title);
        }
      } catch (error) {

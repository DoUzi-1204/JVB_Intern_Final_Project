    ...
    >
      <div className="w-full px-4 flex items-center justify-between gap-4">
        ...
        <nav className="flex items-center gap-6 flex-shrink-0">
                        ...
                        {[...genres.movies, ...genres.tv]
                          ...
                          .map((genre) => (
                            <Link
                              ...
                              to={`/filter?genre=${genre.id}&genreName=${encodeURIComponent(genre.name)}`}
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {POPULAR_MOVIE_COUNTRIES.sort((a, b) =>
                        ...
                        <Link
                          ...
                          to={`/filter?country=${country.iso_3166_1}&countryName=${encodeURIComponent(country.name)}`}
                          className="text-left text-white text-sm py-2 px-3 hover:text-yellow-400 transition-colors duration-200 block"
                          onClick={() => {
                            setShowCountryDropdown(false);
                          }}
          ...
          <Link to="/filter?type=movie" className={navLinkBase}>
            <span>Phim Lẻ</span>
          </Link>

          <Link to="/filter?type=tv" className={navLinkBase}>
            <span>Phim Bộ</span>
          </Link>

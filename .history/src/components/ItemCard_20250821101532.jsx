...
const ItemCard = ({
  ...
  const previewData = useSelector((state) => state.movieDetails[item?.id]);
  const dispatch = useDispatch();
  const [showPreview, setShowPreview] = useState(false);

  // Hàm gọi API lấy previewData
  const fetchPreviewData = async () => {
    if (!previewData && item?.id && mediaType) {
      const API_KEY = import.meta.env.VITE_API_KEY;
      const BASE_URL = API_CONFIG.BASE_URL;
      const endpoint = mediaType === "movie" ? "movie" : "tv";
      const appendResponse = mediaType === "movie" ? "release_dates" : "content_ratings";
      try {
        const viRes = await fetch(`${BASE_URL}/${endpoint}/${item.id}?language=vi-VN&api_key=${API_KEY}&append_to_response=${appendResponse}`);
        const viData = await viRes.json();
        const enRes = await fetch(`${BASE_URL}/${endpoint}/${item.id}?language=en-US&api_key=${API_KEY}&append_to_response=${appendResponse}`);
        const enData = await enRes.json();
        const data = { vi: viData, en: enData, mediaType };
        dispatch(setMovieDetails({ movieId: item.id, data }));
      } catch (err) {
        // handle error
      }
    }
  ...
  const handleMouseEnter = () => {
    setShowPreview(true);
    if (!previewData) fetchPreviewData();
  };

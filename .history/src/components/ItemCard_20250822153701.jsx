...
const ItemCard = ({
  ...
  // Lấy tên tiếng Anh từ item.titleEn (đã gộp khi fetch danh sách phim)
  const titleEn = item?.titleEn || "";

  // ...Bạn thêm phần render UI ở đây, ví dụ:
  return (
    <div className="relative">
      <div>
        <div>{titleVi}</div>
        {titleEn && <div>{titleEn}</div>}
      </div>
      {/* ...các phần UI khác... */}
    </div>
  );
...
const titleEn =
  previewData?.en?.[mediaType === "movie" ? "title" : "name"] || ""; // lấy từ preview khi hover

...
const InforFilm = ({
  ...
  return (
    <div className="max-w-xl space-y-3 sm:text-left text-center sm:items-start items-center flex flex-col">
      {/* Logo */}
        ...
        {getMovieLogo() && (
          <Link to={`/movie/${currentMovie.id}`}>
            <img
              src={getMovieLogo()}
              alt=""
              className="max-w-full max-h-full object-contain hidden xl:block cursor-pointer hover:opacity-80 transition"
            />
          </Link>
        )}
        {!getMovieLogo() && currentDetailsEn?.title && (
          <Link to={`/movie/${currentMovie.id}`}>
            <span className="hidden xl:block text-white text-2xl font-extrabold cursor-pointer hover:underline">
      ...
      <div className="flex flex-col">
        ...
        <Link to={`/movie/${currentMovie.id}`}>
          <span className="xl:hidden text-white text-xl font-semibold mb-0.5 cursor-pointer hover:underline">
        ...
        {currentDetailsEn?.title && (
          <Link to={`/movie/${currentMovie.id}`}>
            <h2 className="text-lg text-yellow-300 font-light mt-0 mb-1 sm:text-left text-center w-full cursor-pointer hover:underline">
              {currentDetailsEn.title}
            </h2>
          </Link>
        )}
      ...
      {/* Movie Info */}
      <div className="flex items-center space-x-2 text-white hidden xsm:flex">

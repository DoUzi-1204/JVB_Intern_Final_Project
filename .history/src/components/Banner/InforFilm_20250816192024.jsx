...
const InforFilm = ({
  ...
  return (
    <div className="flex-1 max-w-2xl space-y-3 text-left">
      ...
      <div className="mb-0.5 w-96 h-24 flex items-center justify-start">
        ...
        {getMovieLogo() ? (
          ...
          <span className="hidden xl:block text-white text-2xl font-bold">
            {currentDetailsEn?.title}
          </span>
        )}
        {/* nhỏ hơn xl: chỉ hiển thị tên tiếng Anh, không hiển thị logo */}
        {currentDetailsEn?.title && (
          <span className="xl:hidden text-white text-xl font-semibold">
            {currentDetailsEn.title}
          </span>
        )}
      </div>

      {/* Title nhỏ luôn hiển thị bên dưới logo */}
      {currentDetailsEn?.title && (
        <h2 className="text-lg text-yellow-300 font-light mb-1 text-left">
          {currentDetailsEn.title}
        </h2>
      )}

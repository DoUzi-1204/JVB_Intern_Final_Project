...
const InforFilm = ({
  ...
  return (
    <div className="flex-1 max-w-2xl space-y-3 text-left">
      {/* Movie Logo (>=xl) hoặc English Title (<xl) */}
  <div className="mb-1 w-96 h-24 flex items-center justify-start">
        {/* xl trở lên: hiển thị logo nếu có */}
        {getMovieLogo() ? (
          <img
            src={getMovieLogo()}
            alt=""
          ...
          <span className="hidden xl:block text-yellow-300 text-2xl font-bold">
            {currentDetailsEn?.title}
          </span>
        )}
        {/* nhỏ hơn xl: chỉ hiển thị tên tiếng Anh, không hiển thị logo */}
        {currentDetailsEn?.title && (
          <span className="xl:hidden text-yellow-300 text-xl font-semibold">
            {currentDetailsEn.title}
          </span>
        )}
      </div>

      {/* English Title luôn hiển thị bên dưới logo */}
      {currentDetailsEn?.title && (
        <h2 className="text-lg text-white font-light mb-2 text-left">
          {currentDetailsEn.title}
        </h2>
      )}

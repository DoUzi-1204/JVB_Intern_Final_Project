  ...
  return (
    <div className="flex-1 max-w-2xl space-y-3 text-left">
      ...
      <div className="mb-2 w-96 h-28 flex items-center justify-start">
        ...
        {/* nhỏ hơn xl: hiển thị cả 2 tên tiếng Anh nếu có */}
        <span className="xl:hidden w-full h-full flex flex-col justify-center">
          {currentDetailsEn?.title && (
            <span className="text-yellow-300 text-xl font-bold">
              {currentDetailsEn.title}
            </span>
          )}
          {currentDetailsEn?.original_title && currentDetailsEn?.original_title !== currentDetailsEn?.title && (
            <span className="text-yellow-200 text-lg font-light">
              {currentDetailsEn.original_title}
            </span>
          )}
        </span>
      </div>

  ...
  return (
    <div className="max-w-xl space-y-3 sm:text-left text-center sm:items-start items-center flex flex-col">
      ...
      <div className="w-96 h-24 flex items-center justify-start mb-2">
        {getMovieLogo() && (
          <Link to={`/movie/${currentMovie.id}`} tabIndex={0} aria-label="Xem chi tiết phim">
            <img
              src={getMovieLogo()}
              alt=""
              className="max-w-full max-h-full object-contain hidden xl:block cursor-pointer"
            />
          </Link>
        )}
        {!getMovieLogo() && currentDetailsEn?.title && (
          <Link to={`/movie/${currentMovie.id}`} tabIndex={0} aria-label="Xem chi tiết phim">
            <span className="hidden xl:block text-white text-2xl font-extrabold cursor-pointer">
      ...
      <div className="flex flex-col">
        ...
        <Link to={`/movie/${currentMovie.id}`} tabIndex={0} aria-label="Xem chi tiết phim">
          <span className="xl:hidden text-white text-xl font-semibold mb-0.5 cursor-pointer">
        ...
        {currentDetailsEn?.title && (
          <Link to={`/movie/${currentMovie.id}`} tabIndex={0} aria-label="Xem chi tiết phim">
            <h2 className="text-lg text-yellow-300 font-light mt-0 mb-1 sm:text-left text-center w-full cursor-pointer">
              {currentDetailsEn.title}
            </h2>
          </Link>
        )}

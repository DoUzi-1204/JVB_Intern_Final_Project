...
const Header = () => {
  ...
  // requests go through server proxy at /api
    ...
    const fetchGenres = async () => {
      ...
      try {
        const [movieRes, tvRes] = await Promise.all([
          fetch(`/api/genre/movie/list?language=vi-VN`),
          fetch(`/api/genre/tv/list?language=vi-VN`),
        ]);
    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 py-3 transition-all duration-300 ${
          isScrolled
            ? "bg-zinc-900 shadow-md backdrop-blur-md"
            : "bg-transparent"
        }`}
      >
        ...
        {isMobileMenuOpen && (
          <nav className="absolute top-full left-0 w-full max-w-[370px] bg-zinc-900/95 backdrop-blur-sm shadow-lg z-50 p-4 xl:hidden">
            {/* Đăng nhập trên cùng */}
            <button
              className={`flex items-center justify-center gap-2 w-full bg-gray-300 rounded-2xl px-3 py-2 mb-4 ${navLinkBase}`}
              onClick={() => {
                setIsMobileMenuOpen(false);
                setIsLoginOpen(true);
              }}
            >
              <UserIcon className="w-5 h-5 flex-shrink-0 text-black" />
...
// Render Login modal at top-level when requested
// (keeps header/footer mounted while modal is open)
// Note: this keeps modal controlled by Header's state
// The Login component accepts onClose prop.

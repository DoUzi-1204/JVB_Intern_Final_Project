...
const Header = () => {
  ...
  // Close dropdowns / mobile menu when route changes to avoid overlays blocking clicks
  useEffect(() => {
    // close dropdowns
    if (genreDropdown?.setIsOpen) genreDropdown.setIsOpen(false);
    if (countryDropdown?.setIsOpen) countryDropdown.setIsOpen(false);
    // close mobile menu and search if open
    if (isMobileMenuOpen) setIsMobileMenuOpen(false);
    if (isSearchOpen) setIsSearchOpen(false);
  }, [location.pathname, location.search, genreDropdown, countryDropdown, isMobileMenuOpen, isSearchOpen]);
            ...
            <div className="hidden xl:flex items-center gap-5 flex-shrink-0 ml-auto">
              <Menu
                ...
                closeAllOverlays={() => {
                  ...
                  if (isMobileMenuOpen) setIsMobileMenuOpen(false);
                  if (isSearchOpen) setIsSearchOpen(false);
                  // blur active element to remove focus traps
                  try {

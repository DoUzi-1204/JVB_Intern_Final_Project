    ...
    >
      <div className="w-full px-4 flex items-center justify-between gap-4">
        ...
        {!isSearchOpen && (
          ...
          <Link
            to="/login"
            className={`flex justify-center items-center gap-2 bg-gray-300 rounded-2xl px-3 py-2 mb-4 ${navLinkBase}`}
          ...
          <div className="flex justify-center items-center mb-4">
            <Link
              to="/login"
              className={`flex justify-center items-center gap-2 bg-gray-300 rounded-2xl px-3 py-2 ${navLinkBase}`}
              onClick={() => setIsMobileMenuOpen(false)}
            >
              <UserIcon className="w-4 h-4 text-black" />
              <span className="text-black font-medium text-sm">Đăng nhập</span>
            {/* Xóa Link đăng nhập thừa ở đây, chỉ giữ ở mobile menu phía dưới */}
                loading={loading}

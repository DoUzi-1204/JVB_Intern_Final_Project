...
const ItemCard = ({
  ...
  // Detect screen size
  const isLg = window.matchMedia("(max-width: 1023px)").matches;
  ...
  return (
    <div
      className="relative"
      onMouseEnter={!isLg ? handleMouseEnter : undefined}
      onMouseLeave={!isLg ? handleMouseLeave : undefined}
    >
      {layout === "horizontal" ? (
        <Link to={`/${mediaType}/${item.id}`}>
          <div className="flex space-x-3 bg-gray-800/50 rounded-lg p-3 cursor-pointer transform transition-all duration-200 hover:bg-gray-700/50 hover:scale-105">
            ...
            <div className="flex-1 min-w-0">
              {titleVi && (
                <h4 className="text-white font-medium text-sm mb-1 truncate leading-tight hover:text-yellow-400 transition-colors">
                  {titleVi}
                </h4>
              )}
              {titleEn && titleVi !== titleEn && (
                <p className="text-gray-400 text-xs truncate leading-tight hover:text-gray-300 transition-colors">
                  {titleEn}
                </p>
              )}
            </div>
        ...
        <div className="w-full max-w-[200px] cursor-pointer transition-transform duration-200 hover:scale-105 group flex flex-col">
          ...
          <Link to={`/${mediaType}/${item.id}`} className="block px-1 mt-2">
            <div style={{ height: "40px" }}>
              {titleVi && (
                <h3 className="text-white font-normal text-sm mb-1 truncate hover:text-yellow-400 transition-colors">
                  {titleVi}
                </h3>
              )}
              {titleEn && (
                <p className="text-gray-400 text-xs truncate hover:text-gray-300 transition-colors">
                  {titleEn}
                </p>
              )}
            </div>
      ...
      {/* Preview Render - Only show for vertical layout and >= lg */}
      {layout === "vertical" && showPreview && previewData && !isLg && (

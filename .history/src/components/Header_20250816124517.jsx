...
const Header = () => {
  ...
  const [isSearchOpen, setIsSearchOpen] = useState(false); // ✅ thêm state search
  ...
  return (
    ...
    >
      <div className="w-full px-4 flex items-center justify-between gap-4">
        {/* Logo + Nav (ẩn khi search mở ở mobile) */}
        {!isSearchOpen && (
          <Link
            to="/"
        ...
        <div
          ...
          } max-w-xs md:max-w-md lg:max-w-lg xl:max-w-md`}
        >
        ...
        {/* Nav desktop - chỉ hiện khi ≥2xl */}
        {!isSearchOpen && (
          <nav className="hidden 2xl:flex items-center gap-6 flex-shrink-0">
            <Dropdown
              label="Thể Loại"
              items={mergedGenres}
            ...
            <Link
              to="/login"
              className={`flex items-center gap-2 bg-gray-200 rounded-2xl px-2 py-0 ${navLinkBase}`}
            >
        ...
        {/* Buttons: Search toggle (mobile) + Menu */}
        <div className="flex items-center gap-2">
          ...
          {/* Hamburger menu - chỉ hiện khi <2xl */}
          {!isSearchOpen && (
            <button className="p-2 rounded-lg bg-gray-700 text-white block 2xl:hidden">
              <svg
                className="w-6 h-6"
                fill="none"
                stroke="currentColor"

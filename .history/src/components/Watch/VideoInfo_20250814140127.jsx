...
const VideoInfo = ({ movieData, type }) => {
  ...
  const getReleaseYear = () => {
    const date = type === 'movie' ? movieData.release_date : movieData.first_air_date;
  ...
  const getTitle = (lang = 'vi') => {
    ...
    return type === 'movie' ? movieData.original_title : movieData.original_name;
  };
  ...
  const getCast = () => {
    return movieData.credits?.cast?.slice(0, 5).map(actor => actor.name).join(', ') || 'N/A';
  };
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center space-x-4 text-sm">
              <div className="flex items-center space-x-1">
                ...
                <span className="text-white">{movieData.vote_average?.toFixed(1)}</span>
              </div>
              <div className="flex items-center space-x-1">
              ...
              {type === 'tv' && movieData.last_episode_to_air && (
                <div className="text-white">
            ...
            <div>
              ...
              <span className="text-white">{getDirector()}</span>
            </div>

            {/* Cast */}
            <div>
              <span className="text-yellow-400 font-medium">Diễn viên: </span>
              <span className="text-white">{getCast()}</span>
            </div>
          </div>
        </div>

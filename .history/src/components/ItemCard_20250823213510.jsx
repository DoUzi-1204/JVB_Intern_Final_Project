...
const ItemCard = ({
  ...
  // Hover timer: show preview only if hovered for >= 2 seconds
  const hoverTimerRef = useRef(null);

  const onMouseEnterDelayed = () => {
    // start 2s timer to trigger preview
  ...
  const titleEn =
    item?.en_title || item?.[mediaType === "movie" ? "title" : "name"] || "";
      ...
      {layout === "horizontal" ? (
        <Link to={`/${mediaType}/${item.id}`}>
          <div className="flex space-x-3 bg-gray-800/50 rounded-lg p-3 cursor-pointer transform transition-all duration-200 hover:bg-gray-700/50 hover:scale-105">
            ...
            <div className="flex-1 min-w-0">
              {titleVi ? (
                <>
                  <h4 className="text-white font-medium text-xs xsm:text-sm mb-1 truncate leading-tight hover:text-yellow-400 transition-colors">
                    {titleVi}
                  </h4>
                  {titleEn && (
        ...
        <div className="w-full max-w-[200px] cursor-pointer transition-transform duration-200 hover:scale-105 group flex flex-col">
          ...
          <Link to={`/${mediaType}/${item.id}`} className="block px-1 mt-2">
            {titleVi && (
              <h3 className="text-white font-normal text-sm mb-1 truncate leading-tight hover:text-yellow-400 transition-colors">
                {titleVi}
              </h3>
            )}
            {titleEn && (

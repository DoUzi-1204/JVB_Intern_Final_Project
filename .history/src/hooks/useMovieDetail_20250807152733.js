    ...
    const fetchMovieDetail = async () => {
      ...
      try {
        ...
        if (!isMovie && detailViData.seasons) {
          ...
          const seasonPromises = detailViData.seasons.map(season => 
            fetch(`${BASE_URL}/tv/${id}/season/${season.season_number}?api_key=${API_KEY}&language=vi-VN`)
              .then(res => res.json())
              .catch(() => null) // Return null if season data fails
          );
          
          const detailedSeasons = await Promise.all(seasonPromises);
          
          // Combine basic season info with detailed data
          seasonsData = detailViData.seasons.map((season, index) => ({
            ...season,
            ...detailedSeasons[index],
            episodes: detailedSeasons[index]?.episodes || []
          }));
        }

...
const useMovieFilter = () => {
        ...
        allResults.sort((a, b) => {
          switch (filters.sortBy) {
            ...
            case "release_date.asc":
              const dateAsc1 = new Date(
                a.release_date || a.first_air_date || 0
              );
              const dateAsc2 = new Date(
                b.release_date || b.first_air_date || 0
              );
  ...
  const handlePageChange = useCallback(
    ...
    [searchParams, setIsReloading, setReloadMessage]
  );
  ...
  const clearFilters = useCallback(() => {
    ...
    // Use window.location.href to navigate to clean filter page
    window.location.href = window.location.origin + "/filter";
  }, [setIsReloading, setReloadMessage]);

  // Tự động fetch dữ liệu khi component mount hoặc khi URL thay đổi
  useEffect(() => {
    if (!isInitialized) {

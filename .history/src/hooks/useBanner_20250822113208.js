...
const useBanner = () => {
  ...
  useEffect(() => {
    const fetchPopularMovies = async () => {
      try {
        const response = await fetch(`${BASE_URL}/movie/popular?language=vi-VN&page=1`);
  ...
  const fetchMovieDetails = useCallback(
    async (movieId) => {
      try {
  const response = await fetch(`${BASE_URL}/movie/${movieId}?language=vi-VN&append_to_response=release_dates`);
        const data = await response.json();

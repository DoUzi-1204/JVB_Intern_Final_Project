...
const useMovieFilter = () => {
  ...
  const navigateWithReload = useCallback(
    (url) => {
      ...
      window.scrollTo(0, 0);
      // Đã loại bỏ trạng thái loading khi điều hướng
    },
    [navigate]
  );
  ...
  const applyFilters = useCallback(() => {
    ...
    const newUrl = params.toString()
      ? `/filter?${params.toString()}`
      : "/filter";
    navigateWithReload(newUrl);
    // Chỉ fetch phim khi nhấn nút lọc
    fetchMovies(filters.currentPage || 1);
  }, [filters, navigateWithReload, fetchMovies]);
  ...
  useEffect(() => {
    if (!isInitialized) {
      ...
      window.scrollTo(0, 0);
      // Không tự động fetch khi searchParams hoặc currentPage thay đổi
    }
  ...
  useEffect(() => {
    if (isInitialized) {
      ...
      // Chỉ cập nhật filters nếu thực sự khác

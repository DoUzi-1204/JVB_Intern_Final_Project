...
const useFilterURL = () => {
  ...
  const navigateToFilter = useCallback((filters, page = 1, replace = false) => {
    const url = createFilterURL(filters, page);
    navigate(url, { replace });
  }, [navigate]);

  // Thêm/cập nhật một filter cụ thể
  const updateFilter = useCallback((filterKey, filterValue, page = 1, replace = true) => {
  ...
  const removeFilter = useCallback((filterKey, page = 1, replace = true) => {
    const currentFilters = getCurrentFilters();
  ...
  // Xóa tất cả filters
  const clearAllFilters = useCallback((replace = true) => {

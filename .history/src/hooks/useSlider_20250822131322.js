...
const useSlider = (endpoint, mediaType, limit = 15) => {
  ...
  // normalize endpoint key to ensure consistent cache keys (no trailing slash)
  const endpointKey = endpoint && endpoint.startsWith("/") ? endpoint : `/${endpoint}`;
  const cachedSlider = useSelector((state) => state.slider[endpointKey]);
  ...
  const fetchData = useCallback(async () => {
    ...
    dispatch(setSliderData({ endpoint: endpointKey, data: allResults }));
      } catch (err) {
  ...
  }, [endpoint, limit, cachedSlider, dispatch]);

  // helpers to interact with redux preview cache
  const cachePreviewData = useCallback(
    (itemId, previewData) => {
      dispatch(setPreview({ id: itemId, data: previewData }));
    },
    [dispatch]
  );
  ...
  const getCachedPreviewData = useCallback(
    (itemId) => {
      return previewCache[itemId] || null;
    },
    [previewCache]
  );
    ...
    async (items) => {
      ...
      const prefetchPromises = items.slice(0, 10).map(async (item) => {
        ...
        try {
          ...
          const resourceType = isMovie ? "movie" : "tv";
          const appendResponse = isMovie ? "release_dates" : "content_ratings";

          // Fetch Vietnamese data
          const viResponse = await fetch(
            `${BASE_URL}/${resourceType}/${item.id}?language=vi-VN&append_to_response=${appendResponse}`
          );
          const viData = await viResponse.json();

          // Fetch English data
          const enResponse = await fetch(
            `${BASE_URL}/${resourceType}/${item.id}?language=en-US&append_to_response=${appendResponse}`
          );
  ...
  [mediaType, previewCache, dispatch]
  );

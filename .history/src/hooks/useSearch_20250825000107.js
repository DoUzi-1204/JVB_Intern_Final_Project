...
const useSearch = () => {
  ...
  // API key is handled by server proxy; client will call /api
    ...
    async (item) => {
      try {
        ...
        const viResponse = await fetch(
          `/api/${endpoint}/${item.id}?language=vi-VN&append_to_response=${appendResponse}`
        );
        const viData = await viResponse.json();

        // Fetch English info with additional data
        const enResponse = await fetch(
          `/api/${endpoint}/${item.id}?language=en-US&append_to_response=${appendResponse}`
        );
        ...
        // For TV series, get current season and episode from last_episode_to_air
        if (item.media_type === "tv") {
          // Use last_episode_to_air from the detailed TV info
          if (enData.last_episode_to_air) {
            currentSeason = enData.last_episode_to_air.season_number;
            currentEpisode = enData.last_episode_to_air.episode_number;
          } else if (enData.number_of_seasons) {
            // Fallback: use latest season and episode 1
    ...
    async (searchQuery, page = 1) => {
      ...
      try {
        const response = await fetch(
          `/api/search/multi?language=vi-VN&query=${encodeURIComponent(
            searchQuery
          )}&page=${page}`
        );

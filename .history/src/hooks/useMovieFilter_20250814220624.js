...
const useMovieFilter = () => {
  ...
  const { getCurrentFilters, getCurrentPage, navigateToFilter } = useFilterURL();
  ...
  const handlePageChange = useCallback(
    (page) => {
      setCurrentPage(page);
      navigateToFilter(filters, page, true);
      fetchMovies(page);
    },
    [fetchMovies, filters, navigateToFilter]
  );

  // Hàm cập nhật filters mà không tự động fetch
  const updateFilters = useCallback((newFilters) => {
    setFilters(newFilters);
    // Không cập nhật URL ngay lập tức, chỉ khi nhấn "Áp dụng"
  }, []);

  // Hàm áp dụng bộ lọc - chỉ gọi khi nhấn nút "Lọc kết quả"
  const applyFilters = useCallback(() => {
    setCurrentPage(1);
    navigateToFilter(filters, 1, true);
    fetchMovies(1);
  }, [fetchMovies, filters, navigateToFilter]);
  ...
  // Đồng bộ filters với URL khi URL thay đổi (chỉ khi khởi tạo hoặc navigation từ bên ngoài)
  useEffect(() => {
    const newFilters = getCurrentFilters();
    const newPage = getCurrentPage();
    
    // Chỉ cập nhật nếu có sự khác biệt thực sự
    ...
    // Chỉ fetch nếu component đã được khởi tạo và có thay đổi
    if (isInitialized && (filtersChanged || pageChanged)) {
      fetchMovies(newPage);
    }
  }, [location.search, getCurrentFilters, getCurrentPage, fetchMovies, isInitialized, filters, currentPage]);
  ...
  }, [fetchMovies, isInitialized, getCurrentPage]);

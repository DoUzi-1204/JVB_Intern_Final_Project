...
export default function useActor({ page = 1, personId = null } = {}) {
  ...
  useEffect(() => {
    if (personId) {
      ...
      setLoading(true);
      Promise.all([
        fetch(`${API_URL}/${personId}?language=vi-VN`).then((res) => res.json()),
        fetch(`${API_URL}/${personId}/combined_credits?language=vi-VN`).then((res) => res.json()),
      ])
  ...
  setLoading(true);
  fetch(`${API_URL}/popular?page=${page}`)
        .then((response) => {
          if (!response.ok) {

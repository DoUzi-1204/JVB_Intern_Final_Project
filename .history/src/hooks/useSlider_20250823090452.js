...
const useSlider = (endpoint, mediaType, limit = 15) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Also fetch English titles/names so callers can show titleEn without hover
      try {
        const allResultsEn = [];
        for (let page = 1; page <= pagesNeeded; page++) {
          ...
          allResultsEn.push(...(pageEn.results || []));
          if (allResultsEn.length >= limit || page >= pageEn.total_pages) break;
  }

  const limitedEn = allResultsEn.slice(0, limit);
        // merge english title/name into the vi items
        const merged = limitedResults.map((it, idx) => {
          const en = limitedEn[idx] || {};
          const titleEnField = mediaType === "movie" ? "title" : "name";
          return {
            ...it,
            titleEn: en[titleEnField] || "",
          };
        });

        setData(merged);
      } catch (errEn) {
        console.error("Error fetching english titles:", errEn);
        // if english fetch fails, fallback to vietnamese-only items
  ...
  }, [endpoint, API_KEY, limit, mediaType]);

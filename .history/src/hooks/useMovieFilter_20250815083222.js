...
const useMovieFilter = () => {
  ...
  const navigateWithReload = useCallback(
    (url, message) => {
      ...
      window.scrollTo(0, 0);
      // Đã loại bỏ window.location.reload để tránh reload vô hạn
    },
    [navigate]
  );
  ...
  useEffect(() => {
    if (isInitialized) {
      ...
      setIsReloading(false); // Đảm bảo tắt trạng thái reloading khi URL thay đổi
    }
  ...
  // Tắt loading khi movies đã fetch xong
};

useEffect(() => {
  if (isReloading && movies && movies.length > 0) {

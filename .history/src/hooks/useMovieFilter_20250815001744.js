...
const useMovieFilter = () => {
  ...
  const handlePageChange = useCallback(
    (page) => {
      ...
      const newUrl = params.toString()
        ...
        : "/filter";
      navigateWithLoading(newUrl, `Đang chuyển đến trang ${page}...`);
    },
  ...
  const applyFilters = useCallback(() => {
    ...
    const newUrl = params.toString()
      ? `/filter?${params.toString()}`
      : "/filter";
    navigateWithLoading(newUrl, "Đang áp dụng bộ lọc...");
  }, [filters, navigateWithLoading]);

  const clearFilters = useCallback(() => {
    navigateWithLoading("/filter", "Đang xóa bộ lọc...");
  }, [navigateWithLoading]);
  ...
  // Khi URL thay đổi, fetch lại dữ liệu và tắt isReloading
  useEffect(() => {
    if (isInitialized) {
      const newFilters = parseFiltersFromURL(searchParams);
      const newPage = parseInt(searchParams.get("page")) || 1;
      ...
      setIsReloading(false); // Tắt trạng thái reloading khi đã fetch xong
      fetchMovies(newPage);
    }
  }, [searchParams, isInitialized, currentPage, fetchMovies]);

  return {
    filters,
    movies,
    isReloading,
    loading, // chỉ trả về 1 trạng thái loading
    reloadMessage,
    currentPage,
    totalPages,
    totalResults,

...
const useBanner = () => {
  ...
  useEffect(() => {
    ...
    // only fetch if not present in caches
    if (!(movieDetails && movieDetails[id])) fetchMovieDetails(id);
    if (!localDetailsEn[id]) fetchMovieDetailsEn(id);
    if (!localImages[id]) fetchMovieImages(id);
  }, [currentMovie, movieDetails, localDetailsEn, localImages, fetchMovieDetails, fetchMovieDetailsEn, fetchMovieImages]);

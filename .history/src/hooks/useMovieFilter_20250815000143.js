...
const useMovieFilter = () => {
  ...
  // Optimize navigation functions - loại bỏ reload để tránh loading 2 lần
  const navigateWithParams = useCallback(
    (url, message) => {
      ...
      // Loại bỏ setTimeout reload để tránh loading 2 lần
  ...
  const handlePageChange = useCallback(
    (page) => {
      ...
      const newUrl = params.toString()
        ...
        : "/filter";
      navigateWithParams(newUrl, `Đang chuyển đến trang ${page}...`);
    },
  ...
  const applyFilters = useCallback(() => {
    ...
    const newUrl = params.toString()
      ? `/filter?${params.toString()}`
      : "/filter";
    navigateWithParams(newUrl, "Đang áp dụng bộ lọc...");
  }, [filters, navigateWithParams]);

  const clearFilters = useCallback(() => {
    navigateWithParams("/filter", "Đang xóa bộ lọc...");
  }, [navigateWithParams]);

  // Optimize useEffect để handle loading state tốt hơn
  useEffect(() => {
    if (!isInitialized) {
  ...
  // Optimize URL parameter tracking với loading state management
  useEffect(() => {
    if (isInitialized) {
      const newFilters = parseFiltersFromURL(searchParams);
      const newPage = parseInt(searchParams.get("page")) || 1;

      // Reset isReloading khi bắt đầu fetch mới
      ...
      // Fetch dữ liệu mới với filters mới
      fetchMovies(newPage);
    }

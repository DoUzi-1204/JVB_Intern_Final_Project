    ...
    async (item) => {
      try {
        ...
        const viResponse = await fetch(
          `${API_BASE}/${endpoint}/${item.id}?language=vi-VN&append_to_response=${appendResponse}`
        );
        const viData = await viResponse.json();

        // Fetch English info with additional data
        const enResponse = await fetch(
          `${API_BASE}/${endpoint}/${item.id}?language=en-US&append_to_response=${appendResponse}`
        );
    ...
    async (searchQuery, page = 1) => {
      ...
      try {
        const response = await fetch(
          `${API_BASE}/search/multi?language=vi-VN&query=${encodeURIComponent(
            searchQuery
          )}&page=${page}`
        );

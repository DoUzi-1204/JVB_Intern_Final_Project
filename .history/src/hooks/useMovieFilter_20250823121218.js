...
const useMovieFilter = () => {
  ...
  const buildDiscoverURL = useCallback(
    (contentType, page = 1, filtersParam = null) => {
      const useFilters = filtersParam || filters;
      ...
      // Add genre filter
      if (useFilters.genres.length > 0) {
        params.append("with_genres", useFilters.genres.join(","));
      }

      // Add country filter
      if (useFilters.countries.length > 0) {
        params.append("with_origin_country", useFilters.countries.join("|"));
      }

      // Add year filter (sử dụng year thay vì date range)
      if (useFilters.selectedYear) {
        if (contentType === "movie") {
          params.append(
            "primary_release_year",
            useFilters.selectedYear.toString()
          );
        } else {
          params.append("first_air_date_year", useFilters.selectedYear.toString());
        }
      }

      // Add certification filter (age ratings) - chỉ áp dụng cho movie
      if (contentType === "movie" && useFilters.ageRatings.length > 0) {
        params.append("certification_country", "US");
        params.append("certification", useFilters.ageRatings.join("|"));
      }
      ...
      try {
        ...
        for (const contentType of contentTypes) {
          ...
          for (
            ...
            const url = buildDiscoverURL(contentType, tmdbPage, effectiveFilters);
            if (!url) continue;
  ...
  // React to react-router location changes (e.g. Link navigation that only updates query)
  const location = useLocation();

  useEffect(() => {
    // Don't run this before initial initialization
    ...
    const urlFilters = {
      ...
      countries: urlParams.get("countries")
        ? urlParams.get("countries").split(",")
        : [],
      selectedYear: urlParams.get("year") ? parseInt(urlParams.get("year")) : null,
      ageRatings: urlParams.get("ageRatings") ? urlParams.get("ageRatings").split(",") : [],
      sortBy: urlParams.get("sortBy") || "popularity.desc",
    };
    ...
    const hasURLParams =
      Object.values(urlFilters).some((value) =>
        Array.isArray(value) ? value.length > 0 : value !== null && value !== "popularity.desc"
      ) || page > 1;

    if (hasURLParams) {
      setFilters(urlFilters);
      setCurrentPage(page);
      // Fetch immediately with the new URL filters (use override to avoid waiting setState)
      setTimeout(() => {
        fetchMovies(page, urlFilters);
      }, 0);
    }
    // If no params, do nothing (keep current filters)
  }, [location.search, isInitialized, fetchMovies]);

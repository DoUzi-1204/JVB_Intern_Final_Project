...
const useMovieFilter = () => {
  ...
  const parseFiltersFromURL = useCallback(() => {
    ...
    return {
      ...
      ageRatings: searchParams.get('age') ? searchParams.get('age').split(',') : [],
  ...
  const updateURLFromFilters = useCallback((filtersObj, page = 1) => {
    ...
    if (filtersObj.contentType) {
      searchParams.set('type', filtersObj.contentType);
    }
    if (filtersObj.genres.length > 0) {
      searchParams.set('genres', filtersObj.genres.join(','));
    }
    if (filtersObj.countries.length > 0) {
      searchParams.set('countries', filtersObj.countries.join(','));
    }
    if (filtersObj.selectedYear) {
      searchParams.set('year', filtersObj.selectedYear.toString());
    }
    if (filtersObj.ageRatings.length > 0) {
      searchParams.set('age', filtersObj.ageRatings.join(','));
    }
    if (filtersObj.sortBy !== "popularity.desc") {
      searchParams.set('sort', filtersObj.sortBy);
    }
    if (page > 1) {
      searchParams.set('page', page.toString());
    }

    const newURL = `/filter${searchParams.toString() ? `?${searchParams.toString()}` : ''}`;

...
const useMovieFilter = () => {
  ...
  useEffect(() => {
    // Khi URL query parameters thay đổi, cập nhật filters và fetch lại dữ liệu
    const newFilters = {
      contentType: searchParams.get("type"),
      genres: searchParams.get("genres") ? searchParams.get("genres").split(",").filter(Boolean) : [],
      ...
      ageRatings: searchParams.get("age") ? searchParams.get("age").split(",").filter(Boolean) : [],
  ...
  // Hàm áp dụng bộ lọc - cập nhật URL query parameters khi nhấn nút "Lọc kết quả"
  const applyFilters = useCallback(() => {
    ...
    if (filters.ageRatings.length > 0) params.set("age", filters.ageRatings.join(","));

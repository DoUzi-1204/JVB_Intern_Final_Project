...
export default function useActor({ page = 1, personId = null } = {}) {
  ...
  useEffect(() => {
    if (personId) {
      ...
      setLoading(true);
      Promise.all([
        fetch(`${API_URL}/${personId}?api_key=${API_KEY}&language=vi-VN`).then(
          (res) => res.json()
        ),
        fetch(
          `${API_URL}/${personId}/combined_credits?api_key=${API_KEY}&language=vi-VN`
        ).then((res) => res.json()),
      ])
      ...
      setLoading(true);
      fetch(`${API_URL}/popular?api_key=${API_KEY}&page=${page}`)
        .then((response) => {
          if (!response.ok) {
